...
const countLabel=n=>`${n} ${n===1?U().piece:U().pieces}`;
...
function addFallbacks(scope=document){
...
function cardMarkup(p,i){
  const title=pText(p,'title'),eyebrow=pText(p,'eyebrow'),story=pText(p,'story'),scope=pText(p,'scope');
  return `<article class="project-card" data-id="${p.id}">
    <div class="project-card-visual" role="button" tabindex="0" aria-label="${U().view.replace(' ↗','')}: ${title}">${imgMarkup(p.files[0],`${title} — 01`,i===0?'eager':'lazy')}</div>
    <div class="project-card-copy">
      <div class="project-card-top"><span class="project-card-no">${String(i+1).padStart(2,'0')}</span><span class="project-card-meta">${eyebrow}</span></div>
      <h3>${title}</h3>
      <p class="project-card-story">${story}</p>
      <div class="project-card-scope"><span>${U().scope}</span><p>${scope}</p></div>
      <div class="project-card-footer"><span>${countLabel(p.files.length)}</span><button class="view-project" type="button">${U().view}</button></div>
    </div>
  </article>`;
}
function archiveMarkup(p,i){
  const title=pText(p,'title'),eyebrow=pText(p,'eyebrow');
  return `<article class="archive-row" data-id="${p.id}"><button class="archive-row-button" type="button" aria-label="${U().view.replace(' ↗','')}: ${title}"><span class="archive-no">${String(i+1+selected.length).padStart(2,'0')}</span><span class="archive-title">${title}</span><span class="archive-meta">${eyebrow}<br>${countLabel(p.files.length)}</span><span class="archive-arrow">↗</span></button></article>`;
}

function renderProjects(){
  $('#selectedProjects').innerHTML=`<div class="deck-shell"><div class="deck-tools"><div class="deck-progress"><span id="deckCurrent">01</span><i></i><span>${String(selected.length).padStart(2,'0')}</span></div><span class="deck-hint">${U().swipe}</span><div class="deck-buttons"><button type="button" id="deckPrev" aria-label="${U().prev}">←</button><button type="button" id="deckNext" aria-label="${U().next}">→</button></div></div><div class="project-deck" id="projectDeck">${selected.map(cardMarkup).join('')}</div></div>`;
  ...
  const closestIndex=()=>{
    ...
    cards.forEach((card,i)=>{const r=card.getBoundingClientRect();const d=Math.abs((r.left+r.width/2)-center);if(d<bestDist){best=i;bestDist=d}});
    return best;
  };
  const updateProgress=()=>{deckIndex=closestIndex();$('#deckCurrent').textContent=String(deckIndex+1).padStart(2,'0')};
  const goTo=i=>{deckIndex=Math.max(0,Math.min(cards.length-1,i));cards[deckIndex].scrollIntoView({behavior:'smooth',inline:'start',block:'nearest'});$('#deckCurrent').textContent=String(deckIndex+1).padStart(2,'0')};
  $('#deckPrev').addEventListener('click',()=>goTo(deckIndex-1));
  $('#deckNext').addEventListener('click',()=>goTo(deckIndex+1));
  let raf=0;deck.addEventListener('scroll',()=>{cancelAnimationFrame(raf);raf=requestAnimationFrame(updateProgress)},{passive:true});
  updateProgress();
}
